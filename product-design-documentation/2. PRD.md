...
* **State Transitions:**
    * When a chore is claimed, it must disappear from the "Available" list and appear in "Claimed by [User]" across all devices (via refetch).
* **Empty States:**
    * If no chores are available, display: *"All caught up! The house is clean (for now)."*
* **Visibility:**
    * Overdue chores must be visually highlighted (e.g., red text or "Overdue" badge) and sorted to the top of the list.

---

## 8. Data & System Constraints
* **Always-Online:** Because the server is the SSOT, the app must show a "Connection Lost" overlay or error if the API is unreachable. No local drafting of chores.
...
* **Double Claim:** User A and User B claim the same chore simultaneously. Server rejects User B. UI must show "Already claimed by User A."
* **User Misuse:** A user completes a chore they didn't do. 
    * *Solution:* Activity log shows "Completed by [User]." Other users can "Unarchive" or "Re-open" if necessary.
* **Network Timeout:** During a "Complete" action, the network drops.
    * *Solution:* Show an error snackbar: "Failed to update. Check your connection and try again."

---

## 11. Open Questions & Assumptions
* **Assumption:** The "Household" is defined by everyone pointing their app to the same API URL/Backend instance. 
* **Question:** Should there be a "Recently Completed" auto-archive timer (e.g., chores move to Archive automatically after 24 hours in Completed)?
  - Answer: No, we want to keep the completed chores visible
* **Risk:** Without push notifications, users might forget to check the app for new chores.

---

## 12. Milestone Roadmap

This roadmap focuses on the user experience and is designed to get a "Minimum Viable Product" into the hands of your household as quickly as possible.

### **Milestone 1: The "Identity"**
* **Goal:** Establish who is using the app.
* **User Value:** I can open the app and see my name at the top.
...
* **User Value:** I can see what needs to be done, claim it, and mark it as finished.
...
* **User Value:** I can see who is slacking and undo my mistakes.
